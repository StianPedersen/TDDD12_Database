...
/*FOREIGN KEYS*/
#ALTER TABLE d_ay ADD CONSTRAINT fk_d_ay FOREIGN KEY (week_id) REFERENCES weekly_schedule(id);
...
set @ticket_number = FLOOR(RAND()*(10000-1+1)+1);
WHILE (@ticket_number IN (SELECT ticket_number FROM traveller)) DO
	set @ticket_number = FLOOR(RAND()*(10000-1+1)+1);
END WHILE;

UPDATE traveller
SET traveller.ticket_number = FLOOR(RAND()*(10000-1+1)+1)
WHERE 
traveller.reservation_id = NEW.reservation_id;
END//
DELIMITER ;
...
set @route_id = (SELECT route_id FROM route WHERE departure_airport_code=departure_airportcode AND arrival_airport_code = arrival_airportcode AND y_ear=route.y_ear);
set @week_id = (SELECT id FROM weekly_schedule WHERE @route_id = route_id AND d_ay=weekly_schedule.d_ay AND t_ime=departure_time AND y_ear=weekly_schedule.y_ear);
set @flight_id = (SELECT flight_id FROM flight WHERE @week_id = week_id AND week=weeknumber);


IF (@week_id IS NULL OR @flight_id IS NULL) THEN
...
ELSE
	...
	ELSE
		...
		VALUES (@reserv_id, @flight_id, pass_number, phone_number, email);
...
DELIMITER //
CREATE DEFINER=`felny523`@`%` FUNCTION `calculatePrice`(flightnumber INT) RETURNS double
BEGIN
set @weekly_id = (SELECT week_id FROM flight WHERE flightnumber = flight_id);
set @route_id = (SELECT route_id FROM weekly_schedule WHERE @weekly_id = id);
set @r_p = (SELECT routeprice FROM route WHERE @route_id = route_id); #ROUTEPRICE

set @d_ay = (SELECT d_ay FROM weekly_schedule WHERE @weekly_id = id);
set @w_f = (SELECT weekdayfactor FROM d_ay WHERE @d_ay = weekday); #WEEKDAYFACTOR
set @booked_passengers = 40-calculateFreeSeats(flightnumber); #BOOKED PASSENGERS

set @y_ear = (SELECT y_ear FROM d_ay WHERE @d_ay = weekday);
set @p_f = (SELECT profitfactor FROM y_ear WHERE @y_ear = y_ear);
